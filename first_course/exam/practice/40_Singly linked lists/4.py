...
class LinkedList:
    ...
    def insert_after_index(self, index, value):
        """Вставка элемента после заданного индекса (0-based)"""
        if index < 0:
        ...
        # Ищем узел с заданным номером
        while current and current_index < index:
            current = current.next
            current_index += 1

        # Если узел нашли, выполняем вставку
        if current:
            new_node = Node(value)
            new_node.next = current.next
            current.next = new_node
        else:
            print(f"Элемент с индексом {index} не найден (выход за границы списка).")
