...
class LinkedList:
    ...
    def find_and_delete(self, value):
        """Поиск и удаление элемента с выводом статуса операции"""
        # Сначала проверяем, существует ли элемент в списке
        node_to_delete = self.find(value)

        if node_to_delete:
            self.delete(value)
            print(f"Элемент {value} успешно найден и удален.")
            return True
        else:
            print(f"Элемент {value} не найден в списке. Удаление невозможно.")
            return False
