...
class LinkedList:
    ...
    def insert_after_value(self, target_value, value):
        """Вставка элемента после узла с заданным значением"""
        # Используем уже существующий метод find для поиска нужного узла
        target_node = self.find(target_value)

        if target_node:
            new_node = Node(value)
            # Новый узел указывает на то, что шло после целевого узла
            new_node.next = target_node.next
            # Целевой узел теперь указывает на новый узел
            target_node.next = new_node
        else:
            print(f"Элемент со значением {target_value} не найден в списке.")
